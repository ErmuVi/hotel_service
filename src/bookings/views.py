...
def create_room(request):
    ...
    try:
        ...
        if not room_description:
            return JsonResponse({"error": "room_description is not defined"}, status=400)

        if not price_room_raw:
            return JsonResponse({"error": "price_room is not defined"}, status=400)
...
def delete_room(request):
    ...
    try:
        ...
        # Валидация: проверяем, что room_id — это целое число
        try:
            room_id = int(room_id_raw)
        except ValueError:
            return JsonResponse({"error": "room_id must be a valid integer number"}, status=400)
...
def create_booking(request):
    ...
    try:
        room_id_raw = request.POST.get("room")
        ...
        try:
            ...
            return JsonResponse({"error": "room ID must be a valid integer number"}, status=400)
        ...
        if date_start >= date_end:
            return JsonResponse({"error": "start_booking cannot be after or equal to end_booking"}, status=400)
...
def delete_booking(request):
    ...
    try:
        ...
        # Валидация: проверяем, что booking_id — это целое число
        try:
            booking_id = int(booking_id_raw)
        except ValueError:
            return JsonResponse({"error": "booking_id must be a valid integer number"}, status=400)
...
def list_bookings(request):
    ...
    try:
        ...
        # Сначала проверим, существует ли вообще такая комната
